...
public class BachelorsProgramFactory(IUser manager) : IEducationProgramFactory
{
    private const int TermsAmount = 8;
    ...
    public EducationProgramResult CreateEducationProgram(string name, IReadOnlyList<ITerm> terms)
    {
        ...
        if (terms.Count != TermsAmount)
        {
            return new EducationProgramResult.EducationProgramMustHaveCertainTerms(
                new BachelorsEducationProgram(name, manager, terms));
        }
        ...
        for (int i = 1; i <= TermsAmount; i++)
        {
            ...
            {
                return new EducationProgramResult.BrokenTermsOrder(new BachelorsEducationProgram(name, manager, terms));
            }
        }

        return new EducationProgramResult.Success(new BachelorsEducationProgram(name, manager, terms));
    }
}